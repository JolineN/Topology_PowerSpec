...
fig, axes = plt.subplots(nrows = 2, ncols=1, figsize=(5, 6), dpi=400, constrained_layout=True)
...
axsLeft[1].plot(l_max_list, kl, color=blue, label = r'$E_2$: $L_B = 1.1 L_{\mathrm{circle}}$')

l_max_list = [10, 20, 30, 40, 50]
kl = np.load('../kl_runs/E2_noncubic_LxLy14_x035_y0/kl_Lz06426_lmax_10_50.npy')
print(kl)
axsLeft[1].plot(l_max_list, kl, color=red, label = r'$E_2$: $L_B = 0.9 L_{\mathrm{circle}}$')
...
for i, l_max in enumerate(l_max_list):
  ...
  axsLeft[0].plot(L, kl, color=red, label=r'$E_1$: No tilt' if i == 0 else None, alpha = 1-0.3*i, linestyle = linestyles[i])

for i, l_max in enumerate(l_max_list):
  kl = np.load('../kl_runs/E2_noncubic_LxLy14_x035_y0/kl_lmax{}.npy'.format(l_max))
  axsLeft[0].plot(L/0.714, kl, color=blue, label=r'$E_2$: $x_0=(0.35, 0, 0)L_{\mathrm{LSS}}$' if i == 0 else None, alpha = 1-0.3*i, linestyle = linestyles[i])
...
axsLeft[0].set_title(r'KL Divergence, $L_A=1.4L_{\mathrm{LSS}}$')
axsLeft[1].text(33, 10, r'$L_A=1.4 L_{\mathrm{LSS}}$')
axsLeft[1].text(33, 15, r'$x_0=(0.35, 0, 0)L_{\mathrm{LSS}}$')
axsLeft[1].legend(frameon=False)
...
axsLeft[0].axvline(1, ymax=0.7, linestyle='dashdot', color = 'black')
axsLeft[0].axhline(1, linestyle='dashdot', color = 'black')
axsLeft[0].set_yscale('log')
axsLeft[0].set_xlabel(r'$L_B / L_{\mathrm{circle}}$')
axsLeft[0].set_ylabel(r'$D_{\mathrm{KL}}$')
axsLeft[0].set_ylim([0.02*1e-1, 3*1e3])
axsLeft[0].set_xlim([0.5, 1.4])
axsLeft[0].yaxis.set_ticks_position('both')
plt.savefig('yashar_right.pdf')

fig, axsRight = plt.subplots(nrows = 2, ncols=1, figsize=(5, 6), dpi=1000)
...
axim = plot_ax(filename, 0, l_min, l_max, lp_min, lp_max, r'$E_2$: $L_B = 0.9L_{\mathrm{circle}}$')
...
axim = plot_ax(filename, 1, l_min, l_max, lp_min, lp_max, r'$E_2$: $L_B = 1.1L_{\mathrm{circle}}$')

cbar_ax = fig.add_axes([0.825, 0.05, 0.05, 0.875])
clb = fig.colorbar(axim, cax=cbar_ax)
clb.ax.set_title(r"$\frac{C_{\ell m \ell' m'}}{\sqrt{C_\ell C_{\ell'}}}$")

fig.subplots_adjust(left=-1)

plt.savefig('yashar_left.pdf', bbox_inches='tight')