...
#  a_t_E1_cubic = np.load('../kl_runs/E1_noncubic_LxLy14_notilt/a_t_lmax{}.npy'.format(l_max))
#  a_t_lmax[i] = a_t_E1_cubic[-3]
l_max_list = [10, 20, 30, 40, 50]
a_t = np.load('../kl_runs/E2_noncubic_LxLy14_x035_y0/a_t_Lz07854_lmax_10_50.npy')
axsLeft[1].plot(l_max_list, a_t, color=blue)



#a_t_lmax = np.zeros(3)
#for i, l_max in enumerate(l_max_list):
#  a_t_E2_cubic = np.load('../kl_runs/E2_noncubic_LxLy14_x035_y0/a_t_lmax{}.npy'.format(l_max))
#  a_t_lmax[i] = a_t_E2_cubic[5]
#axsLeft[1].plot(l_max_list, a_t_lmax, color=blue, label=r'$L_z=0.8$')

l_max_list = [10, 20, 30, 40, 50]
a_t = np.load('../kl_runs/E1_noncubic_LxLy14_notilt/a_Lz11_lmax_10_50.npy')
axsLeft[1].plot(l_max_list, a_t, color=red)
...
a_t_lmax = np.zeros(5)
for i, l_max in enumerate(l_max_list):
  a_t = np.load('../kl_runs/E1_E2_no_tilt_no_shift_old/a_t_E2_lmax{}.npy'.format(l_max))
  a_t_lmax[i] = a_t[-3]
axsLeft[1].plot(l_max_list, a_t_lmax, color=green, label=r'$E_2$ - On axis')
'''
...
for i, l_max in enumerate(l_max_list):
  a_t_no_tilt = np.load('../kl_runs/cubic_E1_no_tilt/a_t_E1_lmax{}.npy'.format(l_max))
  axsLeft[0].plot(L, a_t_no_tilt[2:], color=red, label=r'$E_1$ - No tilt' if i == 0 else None, linestyle = linestyles[i])
for i, l_max in enumerate(l_max_list):
  a_t_tilt = np.load('../kl_runs/a_t_run_E1_tilt_beta_50/a_t_E1_lmax{}.npy'.format(l_max))
  axsLeft[0].plot(L, a_t_tilt, color=blue, label=r'$E_1$ - $\beta=50^\circ$' if i == 0 else None, linestyle = linestyles[i])
for i, l_max in enumerate(l_max_list):
  a_t_E2 = np.load('../kl_runs/cubic_E2_no_tilt_no_shift/a_t_E2_lmax{}.npy'.format(l_max))
  axsLeft[0].plot(L, a_t_E2[2:], color=green, label=r'$E_2$ - On axis' if i == 0 else None, linestyle = linestyles[i])
'''
num = 12
L = np.linspace(0.3, 1.4, num)
l_max_list = [50, 30, 10]
for i, l_max in enumerate(l_max_list):
  a_t = np.load('../kl_runs/E1_noncubic_LxLy14_notilt/a_t_lmax{}.npy'.format(l_max))
  axsLeft[0].plot(L, a_t, color=red, label=r'E1 - No tilt' if i == 0 else None, alpha = 1-0.3*i, linestyle = linestyles[i])

#for i, l_max in enumerate(l_max_list):
#  a_t = np.load('a_t_lmax{}.npy'.format(l_max))
#  plt.plot(L, a_t, color=blue, label=r'E2 - $x_0=(0.35, 0.35, 0)L_{LSS}$' if i == 0 else None, alpha = 1-0.3*i, linestyle = linestyles[i])
for i, l_max in enumerate(l_max_list):
  a_t = np.load('../kl_runs/E2_noncubic_LxLy14_x035_y0/a_t_lmax{}.npy'.format(l_max))
  axsLeft[0].plot(L/0.714, a_t, color=blue, label=r'E2 - $x_0=(0.35, 0, 0)L_{\mathrm{LSS}}$' if i == 0 else None, alpha = 1-0.3*i, linestyle = linestyles[i])
...
axsLeft[0].set_title(r'Kosowsky Statistics, $L_x=L_y=1.4L_{\mathrm{LSS}}$')
...
plt.savefig('tmp_revised_kosowsky.pdf', bbox_inches='tight')